...
class ASTTranslator:
    ...
    def _translate_call(self, func, args, keywords, starargs=None, kwargs=None, location=None):
        ...
        if isinstance(func, ast.Name) and func.id in FORBIDDEN_TOP_LEVEL_FUNCTIONS:
            ...
            if func.id == 'set':
                ...
                if(meta[:6] == '#META:'):
                    if(meta[6:9] == 'Set' and meta[10:] in BUILTIN_SIMPLE_TYPES):
                        return {'type': 'set', 'elements': [], 'pseudo_type': ['Set', BUILTIN_SIMPLE_TYPES[meta[10:]]]}
                    elif(meta[6:9] != 'Set'):
                	    raise type_check_error('You put a meta comment before a set while it doesn\'t seem to concern a set', location, self.lines[location[0]-1])                
                    elif(meta[10:] not in BUILTIN_SIMPLE_TYPES):
                	    raise type_check_error('The hinted type you gave is not supported for set', location, self.lines[location[0]-1])
                    else:
                	    raise type_check_error('pseudo-python needs you to hint the type when declaring empty set, with the syntax #META:Set,<type>', location, self.lines[location[0]])
                else:
                    raise type_check_error('pseudo-python needs you to hint the type when declaring empty set, with the syntax #META:Set,<type>', location, self.lines[location[0]])
    ...
    def _translate_in(self, element, sequence, location):
        ...
        print(element_node)
        if isinstance(sequence_node['pseudo_type'],list) and sequence_node['pseudo_type'][0] == 'Dictionary':
        	message = 'contains?'
        elif (isinstance(sequence_node['pseudo_type'],list) and sequence_node['pseudo_type'][0] == 'List') or sequence_node['pseudo_type'] == 'String':
        	message ='present?'
        else:
        	message = 'include?'
        return {
    ...
    def _translate_set(self, elts, location):
        ...
        if not elts:
            meta = self.lines[location[0]-1]
            if(meta[:6] == '#META:'):
                if(meta[6:10] == 'Set' and meta[11:] in BUILTIN_SIMPLE_TYPES):
                    return {'type': 'set', 'elements': [], 'pseudo_type': ['Set', BUILTIN_SIMPLE_TYPES[meta[11:]]]}
                elif(meta[6:10] != 'Set'):
                	raise type_check_error('You put a meta comment before a set while it doesn\'t seem to concern a list',
                          location, self.lines[location[0]-1])                
                elif(meta[11:] not in BUILTIN_SIMPLE_TYPES):
                	raise type_check_error('The hinted type you gave is not supported for set',
                          location, self.lines[location[0]-1])
                else:
                	raise type_check_error('pseudo-python needs you to hint the type when declaring empty set, with the syntax #META:Set,<type>',
                          location, self.lines[location[0]])
            else:
                raise type_check_error('pseudo-python needs you to hint the type when declaring empty set, with the syntax #META:Set,<type>',
                      location, self.lines[location[0]])
