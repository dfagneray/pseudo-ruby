from helpers import serialize_type
...
class PseudoRubyNotTranslatableError(PseudoError):
    pass

class PseudoRubyTypeCheckError(PseudoError):
    pass

def cant_infer_error(name, line):
    return PseudoRubyTypeCheckError("pseudo-ruby can't infer the types for %s:\n%s\n" % (name, line),
...
@beautiful_error(PseudoRubyTypeCheckError)
def type_check_error(data, location=None, code=None, wrong_type=None, **options):
    pass

@beautiful_error(PseudoRubyNotTranslatableError)
